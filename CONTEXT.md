...
- Sparse snapshot quality verdicts are observable failures. Sparse captures must not replace
  ...
  root-only or sparse tree as an empty UI. The user-facing `snapshot` dispatch attaches a fallback
  screenshot to a sparse response; internal observations never do, so a polling wait cannot turn an
  unreadable screen into one screenshot per poll.
